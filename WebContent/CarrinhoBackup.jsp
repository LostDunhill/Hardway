...
<body>
	...
	<jsp:include page="NavBar.jsp"/>
		...
		<table id="carrinho">
			...
			<c:if test="${not empty produtosCarrinho}">
				...
				<tr>
					...
					<td>
						<form action="controller.do" method="post">
							...
							<input type="text" name="quantidadeProduto" value="${produto.quantidade}">
						</form>
					</td>
					<td><fmt:formatNumber value="${produto.preco * produto.quantidade}" type="currency"/></td>
					<td>
						<form action="controller.do" method="post">
							<input type="hidden" name="id" value="${produto.id}"/>
							<button class="btn btn-danger btn-xs" type="submit" name="command" value="ExcluirProdutoCarrinho">Excluir</button>
						</form>
					</td>
				</tr>
				</c:forEach>
				...
				<tr>
					...
					<td><a class="btn btn-danger btn-xs" href="controller.do?command=LimparCarrinho">Limpar carrinho</a></td>
		...
		<div class="container" style="margin-top: 20px;">
			...
			<form action="controller.do" method="post">
				<div class="row">
					<div class="form-group col-md-4">
						<input id="cep" type="text" class="form-control" name="cep" required maxlength="12" placeholder="Digite seu CEP" value="${cep}"/>
					</div>
					<div class="form-group col-md-4">
						<button id="btncep" class="btn btn-primary" name="command" value="CarregarCEP">Calcular frete</button>
					</div>
				</div>
			</form>
			...
			<a id="btnvoltar" class="btn btn-default" href="Index.jsp">Continuar comprando</a></span></c:if>
			</div>
		</div>
